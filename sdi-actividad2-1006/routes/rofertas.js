module.exports = function (app, swig, gestorBD) {
    ...
    app.get("/home", function (req, res) {
        ...
        gestorBD.obtOfertasPg(criterio, pg, function (ofertas, total) {
            if (ofertas == null) {
                ...
                gestorBD.obtOfertas(criterioVip, function (ofertasFav) {
                    if (ofertasFav == null) {
                        res.send("Error al listar ");
                    } else {
                ...
                if (req.body.descripcion != "") {
                    if (req.body.precio > 0) {
                        ...
                        if (req.body.destacada == "false") {
                            gestorBD.insertOferta(oferta, function (id) {
                                if (id == null) {
                            ...
                            gestorBD.insertOferta(oferta, function (id) {
                                if (id == null) {
                                    ...
                                    gestorBD.modUsuario(criterio, usuarioD, function (result) {
                                        if (result == null) {
                                            console.log("Error al modificar")
                                        } else {
                                            console.log("Su dinero ha sido modificado");
                                        }
...
//Eliminar una oferta
    ...
    app.get('/usr/deleteOferta/:id', function (req, res) {
        var criterio = {"_id": gestorBD.mongo.ObjectID(req.params.id)};
        gestorBD.deleteOferta(criterio, function (ofertas) {
            if (ofertas == null) {
                console.log("Error al eliminar la oferta")
            } else {
                res.redirect("/usr/listarCreadas" + "?mensaje=Oferta eliminada" +
                    "&tipoMensaje=alert-success ");
            }
        });
    })

    //Marcar una oferta como destacada
    app.get('/usr/marcarDestacada/:id', function (req, res) {
        if (req.session.dinero >= 20) {
            ...
            gestorBD.modOferta(criterio, ofertaN, function (result) {
                if (result == null) {
                    console.log("Error al marcar como destacada");
                } else {
                    var total = req.session.dinero - 20;
                    req.session.dinero = total;
        ...
        if (req.session.dinero >= req.params.precio) {
            ...
            gestorBD.modOferta(criterio, ofertaN, function (result) {
                if (result == null) {
                    ...
                    res.redirect("/home" + "?mensaje=Oferta comprada" +
                        "&tipoMensaje=alert-success ");
                }
            });
        } else {
    ...
    app.get("/usr/buscarOferta", function (req, res) {
        ...
        gestorBD.obtOfertasPg(criterio, pg, function (ofertas, total) {
            if (ofertas == null) {
                ...
                gestorBD.obtOfertas(criterioFav, function (ofertasFav) {
                    if (ofertasFav == null) {
                        res.send("Error al listar ");
                    } else {
                        var respuesta = swig.renderFile('views/home.html', {
